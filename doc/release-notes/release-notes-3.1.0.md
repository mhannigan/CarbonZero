C0 Core version *3.1.0* is now available from:  <https://github.com/carbonzero-project/carbonzero/releases>
...
C0 Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zC0 Staking is here! zC0 staking will be activated on the morning of the 8th of May 2018. With the release of zC0 staking, there are effectively 2 versions of zC0, zC0 minted on the 3.0.6 C0 wallet or lower, and zC0 minted on C0 wallet version or higher. New features in this release will require the use of zC0 v2, zC0 minted on this wallet release 3.1.0 or later. If you currently hold zC0 v1 and wish to take advantage of zC0 staking and deterministic zC0, you will need to spend the zC0 v1 to yourself and remint zC0 v2.
Note: To find your zC0 version, click the privacy tab, then the zC0 Control button then expand the arrows next to the desired denomination.


### Deterministic zC0 Seed Keys

zC0 is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zC0 outside of the wallet that the zC0 had been minted on. zC0 can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zC0 minting

zC0 minting now only requires 1 further mint (down from 2) to mature. zC0 mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zC0 Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zC0 denomination has been spent or not.



C0/zC0 Staking and Masternode Rewards
--------------

### C0, zC0 and Masternode Payment Schedule

To encourage the use of zC0 and increase the C0 zerocoin anonymity set, the C0 payment schedule has been changed to the following:

If a user staking zC0 wins the reward for their block, the following zC0 reward will be: 
- 3 zC0 (3 x 1 denominations) rewarded to the staker, 2 C0 rewarded to the masternode owner and 1 C0 available for the budget. This is a total block reward of 6 C0, up from 5.

If a user staking C0 wins the reward, the following amounts will be rewarded: 
- 2 C0 to the C0 staker, 3 C0 to the Masternode owner and 1 C0 available for the budget. This is a total block reward of 6 C0, up from 5.


### Return change to sender when minting zC0

Previously, zC0 minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most C0 and return the change from a zC0 mint to that address.
...
The visual layout of the C0 Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zC0 minting feature. This is enabled by default and the enablezeromint=0 setting in the carbonzero.conf file will overwrite the GUI option.
-	The percentage of autominted zC0 can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular C0.
...
### Summary
 ...
 The C0 wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the carbonzero.conf file, found in the C0 data directory.
...
* If zC0 backup, auto generated name is `wallet-autozpivbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
C0 Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The C0 client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the C0 support group, which often times sees users say "I didn't spend that zC0". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the C0 masternodes, the monthly budget available to be utilised has been increased to 42,000 C0 / month. This C0 only has the opportunity to be raised once per month (paid to winning proposals) with any unused C0 not created by the blockchain.

### Budget Finalization Fee

The C0 finalization fee for successful proposals has now been reduced, this fee is now 5 C0 down from 50 C0. The total fee outlay for a successful proposal is now a total of 55 C0.
...
-	Enforced v1 zC0 spends to require a security level of 100
-	Updates to zC0 spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the C0-Qt masternodes tab
-	Updated C0-Qt tooltips
...
Zerocoins, or zC0, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zC0 that can be recalculated at any time using the seed. Deterministic zC0 allows for users to backup all of their future zC0 by simply recording their seed and keeping it in a safe place (similar to backing up a private key for C0). The zC0 seed needs to remain in the wallet in order to spend the zC0 after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zC0 data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

C0's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which C0 used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to C0 using the bug bounty program. The modulus is now correctly imported and C0's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zC0 v2: zPoS dzC0 ezC0 external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zC0 balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zC0 status output (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zC0 spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zC0. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zC0. (presstab)
