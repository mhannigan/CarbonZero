...
bool CWalletDB::WriteZC0Seed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZC0Seed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZC0Seed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZC0Seed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZC0Seed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZC0Seed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZC0Count(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZC0Count(uint32_t& nCount)
{
