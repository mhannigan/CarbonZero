// Copyright (c) 2018 The C0 developers
...
CzC0Tracker::CzC0Tracker(std::string strWalletFile)
{
...
CzC0Tracker::~CzC0Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzC0Tracker::Init()
{
...
bool CzC0Tracker::Archive(CMintMeta& meta)
{
...
bool CzC0Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzC0Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzC0Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzC0Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzC0Tracker::GetSerialHashes()
{
...
CAmount CzC0Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzC0Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzC0Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzC0Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzC0Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzC0Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzC0Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzC0Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzC0Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzC0Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzC0Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzC0Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzC0Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzC0Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzC0Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzC0Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzC0Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
