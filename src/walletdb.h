...
// Copyright (c) 2016-2018 The C0 developers
...
public:
    ...
    bool WriteZC0Seed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZC0Seed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZC0Seed_deprecated(uint256& seed);
    bool EraseZC0Seed();
    bool EraseZC0Seed_deprecated();

    bool WriteZC0Count(const uint32_t& nCount);
    bool ReadZC0Count(uint32_t& nCount);
