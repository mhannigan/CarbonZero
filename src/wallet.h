...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 C0 at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zC0 send
enum ZerocoinSpendStatus {
    ZC0_SPEND_OKAY = 0,                            // No error
    ZC0_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZC0_WALLET_LOCKED = 2,                         // Wallet was locked
    ZC0_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZC0_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZC0_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZC0_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZC0_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZC0_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZC0_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZC0_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZC0_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZC0_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZC0_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZC0_SPENT_USED_ZC0 = 14,                      // Coin has already been spend
    ZC0_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZC0_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZC0OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzC0Wallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzC0Tracker> zpivTracker;
    ...
    void setZWallet(CzC0Wallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzC0Tracker>(new CzC0Tracker(strWalletFile));
    }

    CzC0Wallet* getZWallet() { return zwalletMain; }
    ...
    /** zC0 reset */
    boost::signals2::signal<void()> NotifyzC0Reset;
