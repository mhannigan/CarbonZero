...
CzC0Wallet::CzC0Wallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZC0Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZC0Seed_deprecated()) {
                    LogPrintf("%s: Updated zC0 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzC0Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZC0Count(nCountLastUsed);
    else if (!walletdb.ReadZC0Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzC0Wallet::Lock()
{
    seedMaster = 0;
}

void CzC0Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzC0Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZC0(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzC0Wallet::LoadMintPoolFromDB()
{
...
void CzC0Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzC0Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzC0Wallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzC0Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZC0(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzC0Wallet::SeedToZC0(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzC0Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzC0Wallet::GenerateDeterministicZC0(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzC0Wallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzC0Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
